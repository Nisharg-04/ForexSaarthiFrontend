...
import { Search, Plus, RefreshCw, FileText, ArrowLeft } from 'lucide-react';
...
export const TradeDocumentsPage: React.FC = () => {
  ...
  // API Hooks
  const { data, isLoading, isFetching, refetch } = useGetTradeDocumentsQuery(tradeId!, {
    skip: !tradeId,
  });
  ...
  const handleGoBack = () => {
    navigate(`/dashboard/trades/${tradeId}`);
  };
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <button
            ...
            className={cn(
              'flex items-center gap-2 text-sm mb-2 transition-colors',
              isDark
          ...
          <p className={cn('text-sm mt-1', isDark ? 'text-slate-400' : 'text-slate-500')}>
            Manage documents for this trade
          </p>
        </div>
